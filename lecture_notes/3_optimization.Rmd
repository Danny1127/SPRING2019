...
- Quasi-Newton optimizers also exist (e.g. BFGS, L-BFGS, etc)
...
Conjugate gradient methods are relatively quick but also very memory efficient

They were originally developed for quadratic objectives but have been expanded to general non-linear functions with lots of success

--

**Conjugate gradient** search methods have search directions of the form
$$ p_k = - \nabla f(x_k) + \beta_k p_{k-1}$$

--

$\beta_k$ is a scalar that ensures $p_k$ and $p_{k-1}$ are *conjugate*: $p_k^T 
...
<div align="center">
  <img src="figures/trust_region.png" height=400>
</div>
...
There is a special case of the trust region where if we set $B_k$ to zero, the solution to the problem is 
$$p_k = -\frac{\Delta_k \,\nabla \,f_k}{||\nabla\,f_k||}$$
...
Some algorithms are more sensitive to poor scaling (steepest descent) than others (Newton)
...
<div align="center">
  <img src="figures/knitro_algos_1.png" height=500>
</div>
...
Suppose we wish to minimize some function subject to some function of the control variables equaling some value
$$\min_x f(x) \,\,\, \text{subject to:} \,\, c_i(x) = 0$$

--

How does the algorithm know to not violate the constraint?
...
<div align="center">
  <img src="figures/penalty_method_1.png" height=500>
</div>
...
<div align="center">
  <img src="figures/penalty_method_2.png" height=500>
</div>
...
Active set methods encapsulate sequential quadratic programming methods
...
Define this system of equations as $F(x,\lambda)$
...
Active set methods use New ton's method to find the solution $(x^*, \lambda^*)$ of $F(x,\lambda)$
...
Active set methods find ways to reduce the complexity of the optimization routine by selectively ignoring constraints that are not active (i.e. non-positive Lagrange multipliers) or close to being active
...
The solution to the barrier problem converges to that of the original constrained optimization problem
...
Optimization is 53% art

--

Not all algorithms are suited for every problem $\rightarrow$ it is useful  
to check how different algorithms perform
