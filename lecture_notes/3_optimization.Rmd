...
$x^{(k+1)} = x^{(k)} - \frac{x^{(k)}-x_{k-1}}{f(x^{(k)})-f(x_{k-1})}f(x^{(k)})$
...
Root guess update is the same as before but with our guess of the Jacobian
...
$B^{(k+1)} = B_{k} + \frac{[d^{(k)} - u^{(k)}]{d^{(k)}}^TB^{(k)}}{{d^{(k)}}^T u^{(k)}}$

where $d^{(k)} = (\mathbf{x^{(k+1)}} - \mathbf{x^{(k)}})$, and $u^{(k)} = B^{(k)}\left[f(\mathbf{x^{(k+1)}})-f(\mathbf{x^{(k)}})\right]$.
...
- $x_{ij}$: the amount of the good traded from region $i$ to region $j$
...
If it's positive: incentive to ship more goods to region $j$ from region $i$
...
<div align="center">
  <img src="figures/complementarity.png" height=550>
</div>
